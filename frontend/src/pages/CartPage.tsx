...
const CartPage = () => {
  const { cart, removeFromCart, updateQuantity, restaurantId } = useCart();
  const total = cart.reduce((sum, item) => sum + item.price * item.quantity, 0);
  const navigate = useNavigate(); // Initialize navigate

  const handleCheckout = () => {
    // Navigate to Index page with cart items and restaurantId as state
    navigate("/app", {
      state: {
        cartItems: cart,
        restaurantId,
      ...
      <main className="max-w-5xl mx-auto px-6 pt-24 pb-12">
              ...
              >
                ...
                <div className="flex items-center gap-4 mt-4 sm:mt-0">
                  <input
                    ...
                    value={item.quantity}
                    onChange={(e) =>
                      updateQuantity(item.id, parseInt(e.target.value) || 1) // Ensure quantity is at least 1
                    }
                    className="border rounded-md px-3 py-1 w-20 text-center text-sm shadow-sm"
                  />
                  <Button
        ...
        {cart.length > 0 && (
          <div className="mt-10 pt-6 border-t flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <Button
              className="bg-[#FF4B3E] hover:bg-[#e34033] text-white px-6 py-2 text-lg"
