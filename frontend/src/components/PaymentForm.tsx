...
const stripePromise = loadStripe('pk_test_TYooMQauvdEDq54NiTphI7jx'); // This is a Stripe test key
...
const CheckoutForm = ({ order, onPaymentComplete }: PaymentFormProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Create a payment intent via the payments API
      ...
      // Use the payment intent client secret to confirm the payment
  ...
  const confirmOrder = async (paymentId: string) => {
    try {
      const token = localStorage.getItem('token');
      await axios.post(`http://localhost:3000/api/orders/${order.id}/confirm`, {
        paymentId,
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="border p-4 rounded-md space-y-4">
        ...
        <div className="text-sm text-gray-500">
          <p className="mb-2">This is a test integration. Use these test cards:</p>
